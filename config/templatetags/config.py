...
def highlight_search(text, keyword):
    # Raw string notatation of 'r' allows us to handle backslashes in non-special way in a string literal.
    # * = 0 or more repetitions of the preceding RE
    # . = any character except a newline
    full_reg = re.compile(
        r'\b{}\b'.format(keyword), re.IGNORECASE)
    part_reg = re.compile(
        r'{}'.format(keyword), re.IGNORECASE)

    try:
        replacement = re.search(full_reg, text).group()
        highlighted = text.replace(
            replacement, '<span class="highlight full">{}</span>'.format(replacement))
        return mark_safe(highlighted)
    except:
        try:
            replacement = re.search(part_reg, text).group()
            highlighted = text.replace(
                replacement, '<span class="highlight part">{}</span>'.format(replacement))
            return mark_safe(highlighted)
        except:
